# Refactoring (while GREEN only) — C#
...
Corollary: run `dotnet test` after every refactor step, even a small one. The whole point of having tests is that they're a cheap detector. Use them.
...
- The same DTO → entity mapping written in two endpoints.
- The same validation block at the top of three actions.
- The same `Ok(new { ... })` / `Created(...)` shape constructed across endpoints.
- The same `using var transaction = ...` pattern in three services.

Extract a helper (an extension method, a static helper, a private method) only when the duplication is genuine and the extracted name is at least as clear as the inlined version. If the helper has to be named `HandleStuff` or `ProcessData`, the abstraction isn't real yet — wait.

### 2. Module depth (see [deep-modules.md](deep-modules.md))

After implementing the slice, look at the public surface. Did the slice grow a thin layer (a wrapper that just forwards to its dependency)? Either fold it into its caller, or deepen it by absorbing nearby logic. A shallow service with one caller is almost never worth keeping.
...
- Did a method grow a side effect its name doesn't suggest (e.g. `GetDocument` now also bumps a counter)? Split, or rename to admit the effect.

### 4. Long methods

A method above ~30 lines, especially with several nested `if`s or a long LINQ chain, is a candidate. Don't split mechanically — look for natural seams (a comment block introducing a "phase", a chunk that operates on a sub-shape of the input) and extract those.
...
- The new endpoint introduced a clean validation pattern using `FluentValidation`. The old endpoint that does it inline now looks worse than it did yesterday — and is worth a same-PR cleanup.
- The new service exposed that a "common" repository method was actually doing two unrelated things. Splitting it now makes both callers clearer.
...
- **Toward speculative abstractions.** "We might need an `IStrategy<>` interface here someday" is not a reason. Wait for the second caller.
- **For style alone.** `dotnet format` is fine; large structural moves to satisfy a style preference are not.
...
3. Run `cd backend/api && dotnet test`.
